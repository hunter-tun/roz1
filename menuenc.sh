...
clear
# Link Installation
link="https://raw.githubusercontent.com/hunter-tun/epep/main"
...
exp=$(curl -sS ${link}/dataroz/license | grep $license1 | awk '{print $2}')
	if [[ $exp < $today ]]; then
		Exp2="\033[1;31mExpired\033[0m"
    else
    Exp2=$(curl -sS ${link}/dataroz/license | grep $license1 | awk '{print $2}')
	fi
d1=$(date -d "$exp" +%s)
d2=$(date -d "$today" +%s)
dayexp=$(( (d1 - d2) / 86400 ))
...
2) clear ; donw ;;
x) clear ; menu ;;
*) echo "salah ketik kontol" ; sleep 0.5 ; roztun ;;
esac
